...
void Algorytm_genetyczny::stworz_populacje()
{
    for (int i = 0; i < Stale::instance()->getIlosc_populacji(); ++i)
    {
        ...
        for (int j = 0; j < Stale::instance()->getIlosc_bitow_x(); ++j)
        {
            chrom.getGeny().push_back(rand() % 2);
        }

        for (int j = 0; j < Stale::instance()->getIlosc_bitow_y(); ++j)
        {
            chrom.getGeny().push_back(rand() % 2);
        }
...
void Algorytm_genetyczny::wyswietl_populacje_10()
{
    ...
    for (vector<Chromosom>::iterator it = populacja.begin(); it != populacja.end(); ++it) //zmiana na iterator
    {
        cout <<"x: "<< it->getReprezentacja10_x()<<"y: "<< it->getReprezentacja10_y();
        cout << endl;
    }
    cout << endl;
}
...
void Algorytm_genetyczny::wyswietl_populacje_przeskalowana()
{
    ...
    for (vector<Chromosom>::iterator it = populacja.begin(); it != populacja.end(); ++it) //zmiana na iterator
    {
        cout << "x: " << it->getPrzeskalowana_wartosc_x() << "y: " << it->getPrzeskalowana_wartosc_y();
        cout << endl;
    }
    cout << endl;
}
...
void Algorytm_genetyczny::inwersja()
{
    ...
    for (auto i = populacja.begin(); i != populacja.end(); ++i)
    {
        ...
        if (losowanie == 1)
        {
            while (1)
            {
                miejsce1 = rand() % suma_bitow;
                miejsce2 = rand() % suma_bitow;
...
Chromosom Algorytm_genetyczny::najlepszy_chromosom()
{
    ...
    {
        //cout << "x: " << najlepszy->getPrzeskalowana_wartosc_x() << "\ty: " << najlepszy->getPrzeskalowana_wartosc_y() << "\tf(x): "<<najlepszy->getWartosc_funkcji_celu() << endl;
        plik_najlepsze << "x: " << najlepszy->getPrzeskalowana_wartosc_x()<<"\ty: " << najlepszy->getPrzeskalowana_wartosc_y() << " f(x): " << najlepszy->getWartosc_funkcji_celu() << endl;
        return *najlepszy;
    }
    else
    {
...
void Algorytm_genetyczny::najgorszy_chromosom()
{
    ...
    if (Stale::instance()->getOptimum() == MAKSIMUM)
    {
        plik_najgorsze << "x: " << najgorszy->getPrzeskalowana_wartosc_x() << "y: " << najgorszy->getPrzeskalowana_wartosc_y() << " f(x): " << najgorszy->getWartosc_funkcji_celu() << endl;
    }
    else
    {
        plik_najgorsze <<"x: " << najgorszy->getPrzeskalowana_wartosc_x() << "y: " << najgorszy->getPrzeskalowana_wartosc_y() << " f(x): " << -(najgorszy->getWartosc_funkcji_celu()) << endl;
